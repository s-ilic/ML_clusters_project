...
hdul0 = fits.open(pathData+'HFI_PCCS_SZ-union_R2.08.fits')
hdul1 = fits.open(pathData+'redmapper_dr8_public_v5.10_catalog.fits')
...
# Grab indexes of redmapper clusters with SZ counterpart
ix_in_RM = []
ix_in_SZ = []
for i, n in enumerate(hdul0[1].data['REDMAPPER']):
    if n != '':
        g = np.where(n == hdul1[1].data['NAME'])[0]
        if len(g) == 0:
            print("WARNING ! i=%s, name=%s" % (i, n))


all_names = list(hdul0[1].data['REDMAPPER'])
for i, n in enumerate(hdul1[1].data['NAME']):
    if n in all_names:
        ix_in_RM.append(i)
        ix_in_SZ.append(all_names.index(n))
