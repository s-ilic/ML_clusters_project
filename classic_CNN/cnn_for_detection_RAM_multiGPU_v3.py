...
# batch_size = 16
# n_want_train = batch_size * 351 * 3
# n_want_valid = batch_size * 351
# n_want_test = batch_size * 351
# batch_size = 32
# n_want_train = batch_size * 175 * 3
# n_want_valid = batch_size * 175
# n_want_test = batch_size * 175
# batch_size = 64
# n_want_train = batch_size * 87 * 3
# n_want_valid = batch_size * 87
# n_want_test = batch_size * 87
# batch_size = 128
# n_want_train = batch_size * 43 * 3
# n_want_valid = batch_size * 43
# n_want_test = batch_size * 43
batch_size = 256
n_want_train = batch_size * 21 * 3
n_want_valid = batch_size * 21
n_want_test = batch_size * 21
...
assert len(final_train) == n_want_train
assert len(final_valid) == n_want_valid
assert len(final_test) == n_want_test
...
with mirrored_strategy.scope():
    ...
    # model.add(Dropout(0.01, name="dropout_001a"))
    # model.add(Conv2D(32, (3, 3), padding='same'))
    # model.add(BatchNormalization())
    # model.add(Activation('relu'))
    # model.add(Dropout(0.01, name="dropout_001b"))
    ...
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    # model.add(Dropout(0.005, name="dropout_0005a"))
    # model.add(Conv2D(64, (3, 3), padding='same'))
    # model.add(BatchNormalization())
    # model.add(Activation('relu'))
    # model.add(Dropout(0.005, name="dropout_0005b"))
    ...
    # model.add(BatchNormalization())
    # model.add(Activation('relu'))
    # model.add(Dropout(0.0025, name="dropout_00025a"))
    # model.add(Conv2D(128, (3, 3), padding='same'))
    # model.add(BatchNormalization())
    # model.add(Activation('relu'))
    # model.add(Dropout(0.0025, name="dropout_00025b"))
...
Y_train[:(n_want_train // 2)] = 1.
Y_valid[:(n_want_valid // 2)] = 1.
Y_test[:(n_want_test // 2)] = 1.

if __name__ == '__main__':
    pool = multiprocessing.Pool(32)
    X_train = np.array(list(
        tqdm(
            pool.imap(get_img, final_train),
    ...
    X_valid = np.array(list(
        tqdm(
            pool.imap(get_img_no_shift, final_valid),
            total=len(final_valid),
            smoothing=0.,
        )
    ))
    X_test = np.array(list(
        tqdm(
            pool.imap(get_img_no_shift, final_test),
...
# prefix = "detect_1024x1024_shift100_Xception_v1"
# prefix = "detect_1024x1024_shift100_CNN_v6"
prefix = "detect_512x512_shift100_CNN_v3"

tf.keras.utils.plot_model(
    model,
    to_file="saved_models/%s_model.png" % prefix,
    show_shapes=True,
)

with open("saved_models/" + prefix + '_summary.txt','w') as fh:
    model.summary(print_fn=lambda x: fh.write(x + '\n'))

np.savez(
    "saved_models/%s_ixs" % prefix,
    final_train=final_train,
    final_valid=final_valid,
    final_test=final_test,
)

log_filename = "saved_models/%s.log" % prefix
...
chk_filename = "saved_models/" + prefix + "_{epoch:02d}-{val_loss:.2f}"
...
'''
model.save_weights('saved_models/weights_%s.h5' % suff)
eval_test = model.evaluate(X_test, Y_test)
np.savez(
    'saved_models/hist_%s' % suff,
