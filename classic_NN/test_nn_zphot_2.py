...
ann.add(Dense(500, input_dim=n_feat, kernel_initializer=my_w1, bias_initializer=my_b1))
ann.add(Activation('relu'))
def my_w2(shape, dtype=None):
    return np.load('z_l2b.npz')['w']
def my_b2(shape, dtype=None):
    return np.load('z_l2b.npz')['b']
ann.add(Dense(1, kernel_initializer=my_w2, bias_initializer=my_b2))
...
plt.axvline(0.39, ls='--', color='red')
plt.axvline(0.41, ls='--', color='red')
plt.xlabel('z spectro')
plt.ylabel('z photo inferred by ANN')
g = (Y_test[:, 0] >= 0.39) & ((Y_test[:, 0] <= 0.41))
plt.subplot(2,2,3)
plt.hist(Y_test_from_ann[g, 0], bins=20)
...
plt.axvline(0.41, ls='--', color='red')
plt.subplot(2,2,4)
g = (Y_train[:, 0] >= 0.39) & ((Y_train[:, 0] <= 0.41))
plt.hist(Y_train_from_ann[g, 0], bins=20)
plt.show()
