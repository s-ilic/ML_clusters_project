...
input_file = [
    'inn_lambda_nmm-10_nperm-128_nshft-64_f0-128_f1-128_r0-128_act-tanh_df-0p5_dr-0p5_l2f-none_l2r-none_loss-mean_squared_error_lr-0p0001', #01
][ix]

spl = input_file.split('_')
if spl[1] == 'lambda':
    labs = ['LAMBDA']
elif spl[1] == 'log10lambda':
    labs = ['log10_LAMBDA']
tmp_spl = spl[2].split('-')
...
ix_keep_clu = [i for i in range(n_clus_tot)]
...
f = np.load(
    'saved_models/'
    'inn_lambda_nmm-10_nperm-128_nshft-64_f0-128_f1-128_r0-128_act-tanh_df-0p5_dr-0p5_l2f-none_l2r-none_loss-mean_squared_error_lr-0p0001/'
    'inference.npz',
    allow_pickle=True,
)
...
plt.xlabel('LAMBDA from redmapper')
plt.ylabel('LAMBDA from NN')
...
plt.plot([mi, ma], [mi, ma], color='red')
plt.tight_layout()
plt.savefig('versus_lambda.pdf')

plt.figure(figsize=(5,5))
err = clu_full_data['LAMBDA_ERR'][ix_valid]
...
plt.xlabel('(LAMBDA_NN - LAMBDA_redmapper)')
# plt.yscale('log')
plt.tight_layout()
plt.savefig('error_lambda.pdf')
...
for p, ls in zip([2.5, 16, 50, 84, 97.5], ['-.','--','-','--','-.']):
    plt.axvline(np.percentile(tp, p), color='black', ls=ls, alpha=0.5)
plt.xlabel('(LAMBDA_NN - LAMBDA_redmapper)/err_LAMBDA_redmapper')
# plt.yscale('log')
plt.tight_layout()
plt.savefig('rerror_lambda.pdf')
...
plt.errorbar(x, y, xerr=xerr,yerr=yerr,fmt='.',ms=0)
...
plt.axis('equal')
plt.tight_layout()
plt.savefig('error_lambda_bis.pdf')
