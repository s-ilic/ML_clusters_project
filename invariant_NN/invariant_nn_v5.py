...
model.load_weights('saved_models/inv_nn_v5/inv_nn_v5')

log_filename = "saved_models/inv_nn_v5/inv_nn_v5.log"
...
chk_filename = "saved_models/inv_nn_v5/inv_nn_v5"
...
plt.savefig('loss_piSGD_janop_clus_radecXYZ_3.pdf')
plt.clf()
'''
...
final_pred /= np.sqrt((final_pred**2.).sum(axis=1))[:, None]
sig = np.arccos(np.sum(final_pred * Y_valid, axis=1)) * 180. / np.pi
