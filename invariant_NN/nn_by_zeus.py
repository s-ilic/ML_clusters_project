...
Y_valid = allY[max_ix:]
'''
tab = np.load('data.npz', allow_pickle=True)
X_train = tab['X_train']
X_valid = tab['X_valid']
Y_train = tab['Y_train']
Y_valid = tab['Y_valid']
...
def lnlike(p):
    ...
    return -0.5 * np.sum((res - Y_train)**2./0.01**2.)
...
sampler.run_mcmc(p_start, n_steps)
np.savez('chain', logprob=sampler.get_log_prob(), chain=sampler.get_chain())
...
l = g.sum()
new_start = np.vstack(
    (sampler.chain[g, -1, :], sampler.chain[g, -1, :][:(n_walkers-l), :])
)
for result in sampler.sample(new_start, iterations=100):
    ct += 1
    print('Current step : %s of %s' % (ct, n_steps))
'''
