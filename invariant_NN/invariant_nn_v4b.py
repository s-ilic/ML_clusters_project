...
def generator(inputs, labels, n):
    i = 0
    while True:
        inputs_batch = np.expand_dims([inputs[i%n]], axis=2)
        labels_batch = np.array([labels[i%n]])
...
inputs = keras.Input(shape=(None, janossy_k * n_feat))
lay1 = Dense(
    num_neurons_in_f,
    # activation="tanh",
    # activation="relu",
...
history = model.fit_generator(
    generator(X_train, Y_train, n_train),
    validation_data=generator(X_valid, Y_valid, n_valid),
    steps_per_epoch=1,
    validation_steps=1,
    epochs=nb_epoch,
)
